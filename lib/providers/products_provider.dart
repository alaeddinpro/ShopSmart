...
class ProductsProvider with ChangeNotifier {
  ...
  ProductsModel? findById(String productId) {
  ...
  List<ProductsModel> findbycategory({required String categoryName}) {
    List<ProductsModel> categoryList = products
        .where(
  ...
  List<ProductsModel> searchQuery({
    ...
    required List<ProductsModel> passedList,
  }) {
  ...
  Future<List<ProductsModel>> fetchProducts() async {
    try {
      await productDb.get().then((productSnapshot) async {
        products.clear();
        for (var element in productSnapshot.docs) {
          ...
          products.insert(
            ...
            ProductsModel(
              ...
              productQuantity: data["productQuantity"]?.toString() ?? '0',
            ),
          );
        }
      });
  ...
  Stream<List<ProductsModel>> fetchProductsStream() {
    try {
